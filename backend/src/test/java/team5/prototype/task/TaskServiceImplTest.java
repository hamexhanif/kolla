...
class TaskServiceImplTest {
    ...
    void createTaskThrowsWhenDefinitionMissing() {
        ...
        when(definitionRepository.findByIdAndTenantId(999L, tenant.getId())).thenReturn(Optional.empty());
    ...
    void getManagerDashboardAggregatesCounts() {
        ...
        when(taskRepository.findAllByTenantId(tenant.getId())).thenReturn(List.of(openTask, dueToday, completed));
    ...
    void getAllTasksAsDtoMapsSteps() {
        ...
        when(taskRepository.findAllByTenantId(tenant.getId())).thenReturn(List.of(task));
